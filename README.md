...
- unlike `Stock`, a `buy.csv` row carries no `price_of_unit` — its market price is always that day's own fetched `yfinance` close, not a manually recorded price. Instead it carries `amount_of_units` alongside a `unit` column (`'troy_ounce'` or `'gram'`), converted internally (via grams) to whichever unit that symbol's own `yfinance` ticker actually quotes a price per — troy ounce for gold/silver/platinum/palladium, pound for copper — so a buy recorded in either supported unit lands on the same cost basis. `sell.csv` is unaffected and still carries its own `price_of_unit`, a real, known sale price
