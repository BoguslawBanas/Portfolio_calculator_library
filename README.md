...
- ~~apply currency conversion to `PolishRetailBonds`~~ — done: it now takes a `currency_to` parameter (converted via `Currency`, defaulting to `'PLN'` — a no-op, so existing callers are unaffected) and `Portfolio` passes its own `currency` through automatically, so a bond's PLN values no longer get summed straight into `Portfolio`'s totals with no FX applied. `PolishRetailBonds` still doesn't expose an `include_native_currency` of its own the way `Stock`/`Commodity`/`Crypto` do, though — see the `Portfolio.include_native_currency` docstring
