...
- a bond stops contributing anything — to the totals and to `distribution_by_ticker`/`_current_value`/`_revenue` alike — the moment it matures; a bond type with every holding past maturity drops out of the distribution entirely rather than lingering at its last-known (stale) value
