...
- the `is_swapped` exchange-price discount (`BOND_TYPES`' `swap_discount`, sourced from each type's *cena zamiany*) and the `cancel.csv` early-redemption fee (`BOND_TYPES`' `early_redemption_fee`) are asserted, not derived from the *listy emisyjne* — bank-quoted exchange pricing and early-redemption fees are neither of them issuance terms, so neither appears in them; double-check both against a current, authoritative source before relying on this for an actual redemption. `TAX_RATE` (19%) is asserted the same way, but is now just a default — override it via the `tax_rate` constructor argument for a situation it doesn't fit
